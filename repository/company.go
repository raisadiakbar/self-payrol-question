...
func (c *companyRepository) CreateOrUpdate(ctx context.Context, company *model.Company) (*model.Company, error) {
	...
	companyModel.Balance = company.Balance

	if err := c.Cfg.Database().WithContext(ctx).Save(companyModel).Error; err != nil {
		return nil, err
	}
...
func (c *companyRepository) DebitBalance(ctx context.Context, amount int, note string) error {
	...
	if err := c.Cfg.Database().WithContext(ctx).Save(company).Error; err != nil {
		return err
	}
